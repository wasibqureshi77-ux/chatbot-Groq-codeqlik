...
def response_generator_node(state: ChatState) -> dict:
    ...
    def _enforce_pending_question(response: str) -> str:
        ...
        Formatting note:
        - The deterministic pending question is placed after a blank line so
          responses feel readable and conversational.
        """
        ...
        # Keep answer content, then append the deterministic pending question.
        kept = []
    ...
    if is_field_answer and not is_info_question and not has_extra_question_after_field(user_text):
        if active_collection and qualified:
            response = (
                " I’ve received the required details.\n\n"
                "Our team can follow up with you shortly. Is there anything else you’d like to know?"
            )
        else:
            ack = build_field_ack(pending_field, active_collection, qualified)
            response = f"{ack}\n\n{build_pending_question(pending_field, active_collection)}"
...
- Use small human micro-phrases only when they fit, like "Yeah", "Okay", "Sure", "Right", "Hmm", "No worries", or "Makes sense".
- Make the response with like a real human breaks use - , ! etc and be the conversation little and funny nature dont be serious.
- Use at most one micro-phrase in a reply.
- Do not use the same opening again and again.
- Light humor is okay only when it feels natural. Do not force jokes.
- Match the user's tone: casual with casual users, clear and professional with serious users, calm with frustrated users.
- Do not use static response like ok , ok or got it , got it again and again in every reply be dynamic.
...
8. CRITICAL FLOW RULE: If Active collection is not None and Pending field is not None, ask ONLY that exact pending field. Never ask exploratory questions, never change the collection order, and never ask for project type/company/budget/timeline unless that exact field is pending.
9. If the latest user message contains both a field value and a company/service question, answer the question first, then continue with the exact pending field.
10. Never ask multiple fields in one reply. Never ask a field already present in Profile.
11. Keep the reply concise and useful. Prefer 1-3 short sentences unless a list is needed for readability.
