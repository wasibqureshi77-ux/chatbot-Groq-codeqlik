...
def retrieve_company_context_details(query: str, limit: int = 4, intent: str = None) -> dict:
    ...
    # Retrieve precomputed embeddings from MongoDB if they exist, otherwise compute them
    doc_embeddings = []
    missing_chunks = []
    missing_indices = []
    ...
    for idx, c in enumerate(candidates):
        ...
        if emb and isinstance(emb, list) and len(emb) > 0:
            ...
            missing_chunks.append(chunks_text[idx])
            missing_indices.append(idx)
            
    if missing_chunks:
        computed_embeddings = embeddings_model.embed_documents(missing_chunks)
        for i, idx in enumerate(missing_indices):
            doc_embeddings[idx] = computed_embeddings[i]
