from typing import Annotated, Optionalfrom typing_extensions import TypedDictimport jsonimport re

from langchain_core.messages import BaseMessage, HumanMessage, AIMessagefrom langgraph.graph import StateGraph, START, ENDfrom langgraph.graph.message import add_messagesfrom langgraph.checkpoint.memory import MemorySaver

from database import (save_chat_to_mongo,save_collection_data,get_chatbot_settings,get_booked_meeting_slots,)from rag.retriever import retrieve_company_context_detailsfrom llm_client import FailoverChatGroq, thread_id_var, node_name_varfrom langsmith import traceable

llm = FailoverChatGroq(model="openai/gpt-oss-20b",temperature=0.4,)

REQUIRED_FIELDS = {"client_lead": ["name","email","phone","company","project_type","requirements","budget","timeline",],"customer_support": ["name","email","phone","issue_type","issue_details","urgency",],"hiring_support": ["name","email","phone","role","experience","skills","resume_or_portfolio",],"meeting_booking": ["name","email","phone","company","work_details","meeting_mode","date","time_slot",]}

COLLECTION_INTENTS = {"client_lead", "customer_support", "hiring_support", "meeting_booking"}

MEETING_TIME_SLOT_OPTIONS = [{"label": "10:00 AM", "value": "10:00 AM"},{"label": "02:00 PM", "value": "02:00 PM"},{"label": "04:00 PM", "value": "04:00 PM"},]

VALID_INTENTS = ["company_info","client_lead","customer_support","hiring_support","meeting_booking","general_chat","unrelated_query",]
...
BUSINESS_DOMAIN_KEYWORDS = ["e commerce", "e-commerce", "ecommerce", "online store", "online shop","shopping platform", "marketplace", "retail", "retail platform","payment gateway", "payment integration", "apple pay", "google pay","razorpay", "stripe", "paypal", "cod", "cash on delivery","cart", "checkout", "order management", "product catalog","inventory", "billing", "invoice", "erp", "business management","management system", "admin panel", "dashboard", "customer portal","vendor portal", "mobile commerce", "m-commerce", "crm", "cms","pos", "booking system", "appointment system", "learning management","lms", "hrm", "hrms", "saas", "automation tools","ai-powered automation", "ai automation", "workflow automation",]

class ChatState(TypedDict):messages: Annotated[list[BaseMessage], add_messages]intent: Optional[str]primary_intent: Optional[str]active_collection: Optional[str]pending_field: Optional[str]user_goal: Optional[str]confidence: Optional[float]profile: dictrequired_fields: list[str]missing_fields: list[str]qualified: boolcompany_context: Optional[str]response_text: Optional[str]thread_id: Optional[str]last_pending_field: Optional[str]current_question: Optional[str]conversation_summary: Optional[str]retrieved_context: Optional[str]rag_confidence: Optional[float]rag_sources: list[str]response_mode: Optional[str]is_field_answer: Optional[bool]saved_collection: Optional[bool]intro_context: Optional[str]slot_conflict: Optional[bool]no_slots_available: Optional[bool]
...
def latest_user_message(messages: list[BaseMessage]) -> str:for msg in reversed(messages):if isinstance(msg, HumanMessage):return msg.contentreturn ""

def format_history(messages: list[BaseMessage], limit: int = 4) -> str:history_msgs = []for msg in messages[-limit:]:role = "User" if isinstance(msg, HumanMessage) or getattr(msg, "type", "") == "human" else "Assistant"history_msgs.append(f"{role}: {msg.content}")return "\n".join(history_msgs)

def safe_json_loads(text: str, fallback: dict) -> dict:try:return json.loads(text)except Exception:pass
...
def normalize(text) -> str:return str(text or "").strip().lower()

INVALID_VALUES = {"","none","null","n/a","na","not provided","unknown","not mentioned","i don't know","dont know","don't know","what","ok","okay","yes","no","sure","go ahead","hmm",}
...
SEMANTIC_COMPAT_ENABLED = TrueSEMANTIC_INTENT_MIN_CONFIDENCE = 0.76

def _space(text: str) -> str:return f" {normalize(text)} "

def _has_any(text: str, phrases: list[str]) -> bool:t = normalize(text)return any(p in t for p in phrases)

def _has_word(text: str, word: str) -> bool:return re.search(rf"(?<![a-z0-9]){re.escape(word)}(?![a-z0-9])", normalize(text)) is not None
...
def is_hinglish_question(text: str) -> bool:t = normalize(text)if not t:return False
...
def has_project_creation_context(text: str) -> bool:t = normalize(text)if not t:return False
...
def has_semantic_client_buying_intent(text: str) -> bool:t = normalize(text)if not t:return False
...
def has_semantic_support_intent(text: str) -> bool:t = normalize(text)if not t:return False
...
def has_semantic_hiring_intent(text: str) -> bool:t = normalize(text)if not t:return False
...
def has_semantic_company_info_intent(text: str) -> bool:t = normalize(text)if not t:return False
...
def looks_like_reference_product(text: str) -> bool:t = normalize(text)if not t:return Falsereference_markers = ["just like", "similar to", "same as", "like ", " clone", "jaisa","jaise", "jesa", "jese", "type", "tarah ka", "tarah ki",]if not any(m in t for m in reference_markers):return False# Short reference replies are common after the bot asks what kind of app/site.return len(t.split()) <= 12 or any(s in t for s in ["app", "website", "software", "platform", "system"])

def extract_reference_requirement_value(text: str) -> Optional[str]:raw = str(text or "").strip()if not raw or not looks_like_reference_product(raw):return None
...
def detect_requested_service_topic(text: str) -> Optional[str]:"""Latest-message service/topic detector with strict priority.
...
def build_service_specific_capability_reply(user_text: str, company_name: str = "CodeQlik", language_hint: Optional[str] = None) -> str:"""Deterministic latest-question answer used only as a safe fallback/guard.It does not ask fields and does not update state. Pending-field enforcementhappens separately in response_generator_node."""topic = detect_requested_service_topic(user_text)lang = language_hint or detect_response_language(user_text)hinglish = lang in {"hinglish", "hindi"}t = normalize(user_text)
...
def should_try_semantic_intent_fallback(user_text: str, rule_intent: str, active_collection: Optional[str]) -> bool:if not SEMANTIC_COMPAT_ENABLED:return Falseif active_collection:return Falseif rule_intent not in {"general_chat", "unrelated_query"}:return Falseif is_small_talk(user_text) or is_sensitive_or_unsafe(user_text):return Falset = normalize(user_text)if not t or len(t.split()) < 2:return Falseif is_plain_value_without_context(user_text):return False# Call LLM only for messages that look like business/user-intent language but# the deterministic rules could not map confidently.return (is_hinglish_question(user_text)or has_project_creation_context(user_text)or looks_like_reference_product(user_text)or has_semantic_company_info_intent(user_text)or any(w in t for w in ["website", "app", "software", "system", "portal", "project", "business", "budget", "price", "support", "job"]))

def has_unrelated_topic_without_project_context(text: str) -> bool:t = normalize(text)unrelated_topics = ["trip", "travel", "tour", "hotel", "flight", "recipe", "cook","pizza", "pasta", "sports", "cricket", "score", "movie", "song","weather", "news", "geography", "history", "capital of","himachal", "goa", "manali", "shimla", "president", "prime minister",]return any(k in t for k in unrelated_topics) and not has_project_creation_context(text)

def safe_semantic_intent_override(user_text: str, rule_intent: str, semantic: dict) -> Optional[str]:intent = semantic.get("intent")confidence = float(semantic.get("confidence", 0.0) or 0.0)if intent not in VALID_INTENTS or confidence < SEMANTIC_INTENT_MIN_CONFIDENCE:return None
...
def is_question(text: str) -> bool:t = normalize(text)return ("?" in tor t.startswith(("what ", "why ", "how ", "have ", "when ", "where ", "who ", "which "))or t.startswith(("can you", "can u", "do you", "does ", "did ", "are you", "is there", "tell me about", "could you", "would you", "should "))or is_hinglish_question(t))

def is_ack(text: str) -> bool:return normalize(text) in {"ok", "okay", "yes", "yeah", "yep", "sure", "go ahead", "fine", "hmm","yes sure", "sure thing", "sounds good"}

def is_small_talk(text: str) -> bool:t = normalize(text)
...
def looks_like_intro_context(text: str) -> bool:"""Temporary intro detector for general_chat.We do NOT save this to profile immediately. We only buffer it and use itif the user later starts client/support/hiring collection."""t = normalize(text)if not t or is_hard_unrelated_or_unsafe(t):return False
...
def is_company_or_business_info_query(text: str) -> bool:"""Company/business/hiring information questions should be answered dynamically.They may use RAG/context, and if a collection is active, the bot should answerfirst and then ask the pending field."""t = normalize(text)
...
def is_explanatory_or_company_question(text: str) -> bool:"""Company/info questions should be answered but must not be stored as profile fields.Examples: explain software development, what is CodeQlik, tell me about services."""t = normalize(text)
...
def is_field_like_answer(text: str, expected_field: Optional[str]) -> bool:"""Blocks normal questions/explanations from being saved into profile.Allows real field values, labeled values, emails/phones, and explicit refusals."""t = normalize(text)
...
def is_gibberish(text: str) -> bool:t = normalize(text)if not t:return True
...
def has_email(text: str) -> bool:return bool(re.search(r"[\w.-]+@[\w.-]+.\w+", text or ""))

def extract_email(text: str) -> Optional[str]:m = re.search(r"[\w.-]+@[\w.-]+.\w+", text or "")return m.group(0) if m else None

def extract_phone(text: str) -> Optional[str]:digits = re.sub(r"\D", "", text or "")if 7 <= len(digits) <= 15:return digitsreturn None

def extract_meeting_mode(text: str) -> Optional[str]:low = normalize(text)if not low:return None
...
def extract_meeting_date(text: str) -> Optional[str]:raw = str(text or "").strip()low = normalize(raw)if not low:return None
...
def extract_time_slot(text: str) -> Optional[str]:low = normalize(text)if not low:return None
...
def is_booking_control_phrase(text: str) -> bool:"""Detect short messages that start/confirm the booking flow, not field values.Example: "book a meeting" should not be saved as the user's name."""low = normalize(text)if not low:return False
...
def looks_like_refusal(text: str) -> bool:t = normalize(text)phrases = ["i don't want","i dont want","i do not want","do not want to share","don't want to share","dont want to share","not comfortable","private","skip","skip this","no thanks","don't ask","dont ask","i won't","i will not","rather not","not interested in sharing","can't share","cannot share","share nahi karna","share nhi karna","nahi batana","nhi batana","mat pucho","skip karo","skip kar do",]return any(p in t for p in phrases)

def is_valid_field_value(value, field_name=None):if value is None:return False
...
def is_answered(value, field_name=None):if value is None:return Falseif normalize(value) in DECLINED_VALUES:return Truereturn is_valid_field_value(value, field_name)

def check_missing_fields(profile: dict, required_fields: list[str]) -> list[str]:profile = profile or {}missing = []for field in required_fields:if not is_answered(profile.get(field), field):missing.append(field)return missing

def sync_collection_state(category: Optional[str], profile: dict) -> dict:if not category:return {"required_fields": [],"missing_fields": [],"qualified": False,"pending_field": None,"last_pending_field": None,"active_collection": None,}
...
def merge_profile(old: dict, new: dict, category: Optional[str] = None) -> dict:merged = dict(old or {})allowed = set(REQUIRED_FIELDS.get(category or "", []))
...
def has_business_domain_keyword(text: str) -> bool:"""Broad, non-lead business vocabulary for company/service info questions."""t = normalize(text)return any(k in t for k in BUSINESS_DOMAIN_KEYWORDS)

def rag_suggests_company_relevance(text: str, threshold: float = 0.40) -> bool:"""Use RAG only as a fallback for ambiguous questions to avoid false refusals."""if not text or is_small_talk(text) or is_hard_unrelated_or_unsafe(text):return Falsetry:details = retrieve_company_context_details(text)confidence = float(details.get("confidence", 0.0) or 0.0)return confidence >= thresholdexcept Exception as e:print("[Relevance RAG] skipped:", e)return False

def llm_suggests_company_relevance(text: str) -> bool:"""Last-resort ambiguity check; never overrides hard unsafe/unrelated topics."""if not text or is_small_talk(text) or is_hard_unrelated_or_unsafe(text):return False
...
Return JSON only:{{"company_related": true, "reason": ""}}"""try:node_name_var.set("relevance_check")json_llm = llm.bind(response_format={"type": "json_object"})result = json_llm.invoke([HumanMessage(content=prompt)]).contentparsed = safe_json_loads(result, {"company_related": False})return bool(parsed.get("company_related", False))except Exception as e:print("[Relevance LLM] skipped:", e)return False
...
def is_company_related(text: str) -> bool:t = normalize(text)keywords = ["codeqlik","company","service","services","pricing","price","cost","contact","phone","email","address","office","portfolio","technology","technologies","project","website","web","app","mobile app","software","crm","saas","automation","ai","chatbot","support","bug","issue","complaint","not opening","crashing","error","intern","internship","job","career","hiring","resume","developer","developers","hire","quote","proposal","budget","timeline","requirements","opening","openings","role","roles","available role","available roles","vacancy","vacancies","who are you","what can you do","what do you do",# Business/project discussion terms that are valid during client lead flow"business management","management system","erp","inventory","billing","invoice","dashboard","admin panel","employee management","attendance","sales","purchase","reporting","analytics","customer portal","vendor portal","accounting","business management system","management software","custom software","business software",]return any((re.search(r"\bai\b", t) is not None) if k == "ai" else (k in t) for k in keywords) or has_business_domain_keyword(t)

def is_sensitive_or_unsafe(text: str) -> bool:t = normalize(text)unsafe = ["system prompt","ignore previous instructions","developer message","api key","secret key","mongodb uri","database password","hack","hacking","malware","phishing","payload","sql injection","exploit","adult","porn","religion","politics","prime minister","president","which llm","model provider","internal instruction",]return any(k in t for k in unsafe)

def is_hard_unrelated_or_unsafe(text: str) -> bool:"""Strong refusal list used while a lead/support/hiring collection is active.During an active collection, broad project questions must NOT be rejectedjust because they are questions. Only clearly unsafe or clearly off-companytopics should be refused.
...
def is_unrelated_query(text: str) -> bool:t = normalize(text)
...
def is_plain_value_without_context(text: str) -> bool:"""Standalone values like "Anurag", "Rahul Sharma", "SalesPro" should not start a flowwhen the bot has not asked for that field."""t = normalize(text)
...
def has_client_buying_intent(text: str) -> bool:"""Detect dynamic client/project buying intent without making every service word a lead.A service term alone (for example, "ERP") is not enough. It needs a buying/help/cost cue.This keeps general info questions like "what is ERP" separate from client leads."""t = normalize(text)if not t or is_small_talk(t) or is_sensitive_or_unsafe(t):return False
...
def has_hiring_info_intent(text: str) -> bool:t = normalize(text)return any(k in t for k in ["opening", "openings", "available role", "available roles","roles available", "vacancy", "vacancies", "hiring","career", "internship", "job"]) or has_semantic_hiring_intent(t)

def classify_intent_semantic(user_text: str, active_collection: Optional[str]) -> dict:"""LLM semantic classifier.It only classifies intent/switch behavior.It must NOT extract profile, decide pending field, or decide qualification.Deterministic state manager still controls the flow."""prompt = f"""You are an intent classifier for CodeQlik's company support chatbot.
...
Current active_collection: {active_collection}Latest user message: {user_text}

Important rules:

Do NOT extract name, email, phone, company, budget, or any profile data.

Do NOT decide next pending field.

If current active_collection exists and user is just answering a field, keep same intent.

If user clearly switches topic, set is_clear_switch=true.

If user asks company/service/hiring info during active_collection, keep same active intent but answer_type should be "company_answer".

New wording should be understood semantically, not only by keywords.

Unsafe/unrelated requests must be unrelated_query.

Return JSON only:{{"intent": "client_lead|customer_support|hiring_support|meeting_booking|company_info|general_chat|unrelated_query","is_clear_switch": true,"answer_type": "field_answer|company_answer|small_talk|refusal","confidence": 0.0,"reason": ""}}"""
...
def classify_intent_rules(user_text: str, active_collection: Optional[str]) -> str:"""Safer hybrid intent classification:- Deterministic state/flow protection first.- Semantic compatibility only maps new wording into the same old intents.- LLM fallback is conservative and only used when rules would otherwise fail.- LLM never controls profile/pending/qualification."""t = normalize(user_text)
...
def is_clear_flow_switch(user_text: str, current_active: Optional[str], new_intent: str) -> bool:if not current_active:return True
...
@traceabledef is_unsupported_meeting_topic(text: str) -> bool:low = normalize(text)unrelated_words = ["trip", "travel", "vacation", "holiday", "tour", "recipe", "cooking", "food","movie", "film", "politics", "election", "religion", "god", "medical", "doctor","health", "legal", "lawyer", "court", "finance", "investment", "stock"]company_keywords = ["website", "app", "software", "development", "crm", "saas", "automation", "codeqlik", "project", "it solutions"]if any(w in low for w in unrelated_words) and not any(k in low for k in company_keywords):return Truereturn False

def build_meeting_prefill_from_profile(profile: dict) -> dict:profile = profile or {}contact_value = profile.get("email_or_phone") or ""email = profile.get("email") or extract_email(contact_value)phone = profile.get("phone") or extract_phone(contact_value)work_details = (profile.get("work_details")or profile.get("requirements")or profile.get("project_type")or profile.get("issue_details")or profile.get("issue_type"))
...
def intent_classifier_node(state: ChatState) -> dict:user_text = latest_user_message(state["messages"])current_active = state.get("active_collection")original_active = current_activecurrent_profile = state.get("profile", {}) or {}summary = state.get("conversation_summary", "")
...
def route_by_intent(state: ChatState) -> str:return state.get("primary_intent") or state.get("intent") or "general_chat"
...
def has_extra_question_after_field(text: str) -> bool:t = normalize(text)return ("?" in tor " can you " in f" {t} "or " can i " in f" {t} "or " explain " in f" {t} "or " know about " in f" {t} "or "how " in tor "what " in tor "why " in tor "when " in tor "where " in tor "which " in tor "who " in tor "have" in tor "tell" in tor is_hinglish_question(t))

def is_context_followup(text: str) -> bool:"""Detect short follow-up questions that refer to already saved profile context.Example after a completed lead/support flow:- what you use in it- how will you create it- how will you fix it- can you explain thisThese should use saved profile context instead of being treated as unrelated."""t = normalize(text)if not t:return False
...
def has_saved_business_context(profile: dict) -> bool:profile = profile or {}return bool(profile.get("project_type")or profile.get("requirements")or profile.get("issue_type")or profile.get("issue_details")or profile.get("role")or profile.get("skills")or profile.get("work_details"))

def remove_label_prefix(text: str, labels: list[str]) -> Optional[str]:raw = str(text or "").strip()
...
def extract_inline_labeled_values(text: str, labels: list[str]) -> list[str]:"""Extract labeled values even when multiple fields are in one sentence:"my name is Karan and company is RetailX""company is SalesPro and project type is CRM""""raw = str(text or "").strip()values = []
...
def validate_extracted_field(key: str, value, category: str) -> Optional[str]:"""Strict validation layer for LLM/regex extracted fields.Rejects accidental long sentences, generic words, and question-like values."""if value is None:return None
...
def validate_extracted_profile(raw: dict, category: str) -> dict:allowed = set(REQUIRED_FIELDS.get(category, [])) | {"email", "phone", "email_or_phone"}clean = {}
...
def llm_verify_uncertain_field(field: Optional[str], user_text: str, category: str) -> Optional[str]:"""Last-resort verifier for uncertain field answers.
...
The chatbot is currently expecting exactly this field:{field}
...
Return JSON only:{{"valid": true/false,"value": "extracted value or null"}}"""
...
def extract_structured_profile_llm(user_text: str, category: str, current_profile: dict, expected_field: Optional[str]) -> dict:"""LLM-based structured extractor.It extracts only explicitly mentioned fields from the latest user message.It does NOT infer missing fields and does NOT decide pending/qualified.Validation runs after this before saving."""if is_small_talk(user_text) or is_ack(user_text) or is_gibberish(user_text):return {}
...
Extract ONLY fields explicitly mentioned in the latest user message.Do NOT guess.Do NOT fill a field just because it is pending.Do NOT put the whole message into a field.If a value is not clearly present, return null for that field.If user refuses/skips a field, use "declined".

Category: {category}Expected pending field: {expected_field}Existing profile: {json.dumps(current_profile or {}, ensure_ascii=False)}Allowed output fields: {list(schema_hint.keys())}
...
Return JSON only:{json.dumps(schema_hint, indent=2)}"""
...
def extract_obvious_profile_regex(user_text: str, category: str) -> dict:"""Deterministic supplement for common natural phrases."""text = str(user_text or "").strip()out = {}
...
def extract_expected_field(user_text: str, expected_field: Optional[str], category: str) -> dict:text = str(user_text or "").strip()low = normalize(text)
...
def extract_semantic_compat_profile(user_text: str, category: str, expected_field: Optional[str]) -> dict:"""Deterministic semantic supplement for Hinglish/new phrasing.It never decides flow or pending fields. It only extracts explicitly presentvalues into the same existing REQUIRED_FIELDS schema."""text = str(user_text or "").strip()low = normalize(text)out = {}
...
def extract_direct_labeled_fields(user_text: str, category: str) -> dict:"""Strong deterministic extraction for explicit labeled values.This runs before generic field-like filtering so cases like:- company is SalesPro- project type is CRM- budget is 50000are not skipped."""text = str(user_text or "").strip()low = normalize(text)out = {}
...
def extract_labeled_fields(user_text: str, category: str) -> dict:"""Only explicit labeled values; no question inference."""if is_question(user_text) or is_gibberish(user_text) or is_ack(user_text):return {}
...
@traceabledef extract_collection_data(state: ChatState, category: str) -> dict:user_text = latest_user_message(state["messages"])current_profile = state.get("profile", {}) or {}required = REQUIRED_FIELDS.get(category, [])intro_context = state.get("intro_context") or ""combined_text = (intro_context + "\n" + user_text).strip() if intro_context else user_text
...
@traceabledef unrelated_query_node(state: ChatState) -> dict:settings = get_chatbot_settings()company_name = settings.get("company_name", "CodeQlik")response = f"I can only help with {company_name}-related services, projects, support, or hiring. Please ask something related to our company."
...
@traceabledef general_chat_node(state: ChatState) -> dict:if state.get("active_collection"):return {}
...
@traceabledef client_lead_node(state: ChatState) -> dict:user_text = latest_user_message(state["messages"])data = extract_collection_data(state, "client_lead")
...
@traceabledef customer_support_node(state: ChatState) -> dict:data = extract_collection_data(state, "customer_support")
...
@traceabledef hiring_support_node(state: ChatState) -> dict:data = extract_collection_data(state, "hiring_support")
...
def repair_meeting_booking_field(state: ChatState, data: dict) -> dict:profile = dict(data.get("profile") or {})required = REQUIRED_FIELDS.get("meeting_booking", [])missing_before = check_missing_fields(state.get("profile", {}) or {}, required)expected_field = missing_before[0] if missing_before else Noneuser_text = latest_user_message(state["messages"])patch = {}
...
@traceabledef meeting_booking_node(state: ChatState) -> dict:user_text = latest_user_message(state["messages"])
...
def build_pending_question(pending_field: Optional[str],active_collection: Optional[str],soft: bool = False,language_hint: Optional[str] = None,profile: Optional[dict] = None,thread_id: Optional[str] = None,) -> str:"""Natural pending-field question builder.
...
def detect_response_language(text: str) -> str:"""Detect the latest user's language in a conservative way.
...
def is_hinglish_style(text: str) -> bool:return detect_response_language(text) in {"hinglish", "hindi"}

def detect_conversation_language(messages: list[BaseMessage], latest_text: str = "") -> str:"""Prefer the latest meaningful user message, then recent user history.This keeps field-only replies like "Anurag" from switching a Hinglishconversation back to English."""latest_lang = detect_response_language(latest_text)latest_words = len(normalize(latest_text).split())
...
def language_instruction(language_hint: str) -> str:if language_hint == "hindi":return ("Reply in Hindi only. Keep company names and technical terms like CodeQlik, CRM, API, dashboard as-is when natural.")if language_hint == "hinglish":return ("Reply in natural Hinglish only, matching the user style. Do not switch to full English unless the user switches to English.")return ("Reply in English only. Do not use Hindi/Hinglish words such as haan, aap, chahiye, ""bana sakta hai, kya, kaise, hoga, or aapke liye. Ignore previous assistant language if it conflicts.")

def response_violates_language(text: str, language_hint: str) -> bool:"""Small deterministic guard for obvious language drift."""if language_hint != "english":return Falselow = normalize(text)english_blocklist = ["haan", "aap", "aapke", "chahiye", "bana sakta", "bana sakte","kar sakta", "kar sakte", "hoga", "kya", "kaise", "liye","me help", "ke saath", "is tarah", "samajh gaya",]for w in english_blocklist:if w.isalpha():if re.search(rf"(?<![a-z0-9]){re.escape(w)}(?![a-z0-9])", low):return Trueelif w in low:return Truereturn False

def is_client_project_detail_turn(text: str) -> bool:"""True when the user is continuing to describe/refine a client project.This is intentionally separate from pending-field collection so projectdetails are not mistaken for name/company while the lead flow is active."""t = normalize(text)if not t or is_small_talk(t) or is_hard_unrelated_or_unsafe(t):return Falseif extract_email(t) or extract_phone(t):return Falseif looks_like_refusal(t):return False# Explicit identity/company labels are handled by normal field extraction.if re.search(r"(?i)\b(my name|mera naam|naam|company name|company ka naam|my company)\b", text):return Falseproject_words = ["website", "web site", "site", "web app", "mobile app", "android", "ios","app", "application", "software", "portal", "platform", "system", "crm","erp", "dashboard", "ecommerce", "e-commerce", "online store", "booking","chatbot", "automation", "design", "develop", "build", "create", "make","banwana", "banwani", "banana", "banani", "bana", "bna", "chahiye","chaiye", "karwa", "karwana", "price", "cost", "estimate", "budget",]return has_project_creation_context(text) or looks_like_reference_product(text) or any(w in t for w in project_words)

def build_client_project_detail_reply(user_text: str, profile: dict, pending_field: Optional[str], active_collection: Optional[str], language_hint: Optional[str] = None) -> str:"""Deterministic response for project-detail/refinement turns while identity/contactfields are still pending. This prevents empty/repetitive responses like onlyasking the name after every project detail.
...
@traceabledef response_generator_node(state: ChatState) -> dict:user_text = latest_user_message(state["messages"])current_question = state.get("current_question") or user_textintent = state.get("intent", "general_chat")primary_intent = state.get("primary_intent", "general_chat")active_collection = state.get("active_collection")pending_field = state.get("pending_field")profile = state.get("profile", {}) or {}missing_fields = state.get("missing_fields", []) or []retrieved_context = state.get("retrieved_context") or state.get("company_context") or ""rag_confidence = state.get("rag_confidence", 0.0) or 0.0rag_sources = state.get("rag_sources", []) or []conversation_summary = state.get("conversation_summary", "") or ""qualified = state.get("qualified", False)is_field_answer = state.get("is_field_answer", False)has_profile_context = has_saved_business_context(profile)is_saved_context_followup = bool(qualified and has_profile_context and is_context_followup(user_text))response_language = detect_conversation_language(state.get("messages", []), user_text)collection_context = active_collectionif not collection_context and qualified and primary_intent in COLLECTION_INTENTS:collection_context = primary_intent
...
Latest user language: {response_language}Language instruction: {language_instruction(response_language)}Active collection: {active_collection}Pending field to collect: {pending_field}Meaning of this field: {_field_description(active_collection, pending_field)}Already collected profile:{json.dumps(question_profile_display, ensure_ascii=False)}Latest user message:"{current_question}"
...
Latest user language: {response_language}Language instruction: {language_instruction(response_language)}Latest user message:"{current_question}"
...
Latest user message:"{current_question}"

Latest user language: {response_language}Language instruction: {language_instruction(response_language)}

Recent history for context only:{_trim(format_history(state["messages"][:-1], limit=5), 900)}

State:Intent: {primary_intent}Active collection: {active_collection}Pending field: {pending_field}Missing fields: {missing_fields}Qualified: {qualified}Profile / saved business context:{json.dumps(profile_display, ensure_ascii=False)}

Company/RAG context, use only if relevant:{_trim(retrieved_context, 1200)}
...
Answer the latest user message naturally and dynamically. Do not repeat the same generic line for every small-talk message.

Stay focused on {company_name}: services, projects, support, hiring, pricing, contact, and company information.

If the user asks casual small talk, reply naturally and briefly, then keep the conversation open for company help.

If the user asks who you are or what you can do, explain your role as {company_name}'s support assistant.

If exact company data is unavailable, answer generally without inventing exact facts.

Use Company/RAG context only if it is relevant to the latest user message. Ignore it if it is not useful.

{pending_instruction}

CRITICAL FLOW RULE: If Active collection is not None and Pending field is not None, ask ONLY that exact pending field. Ask it dynamically and naturally. Never ask for other missing fields.

If the user asks a question, always answer/address the user's question first (using the Company/RAG context if relevant), and then naturally ask for the pending field to continue the collection.

Never ask multiple fields in one reply. Never ask a field already present in Profile.

Keep the reply concise and useful. Prefer 1-3 short sentences ,but if the list is needed for readability then go upto 5-8 short sentences.

Avoid robotic lines like "Please share your name so we can guide you better", "How can I assist you today?", or "Please provide more details."

DO NOT include: Thinking process, Reasoning steps ,Analysis ,Internal instructions, Any text like "Here's a thinking process. Only return the final answer.
...
graph.add_node("intent_classifier", intent_classifier_node)graph.add_node("client_lead", client_lead_node)graph.add_node("customer_support", customer_support_node)graph.add_node("hiring_support", hiring_support_node)graph.add_node("meeting_booking", meeting_booking_node)graph.add_node("general_chat", general_chat_node)graph.add_node("unrelated_query", unrelated_query_node)graph.add_node("response_generator", response_generator_node)

graph.add_edge(START, "intent_classifier")

graph.add_conditional_edges("intent_classifier",route_by_intent,{"company_info": "client_lead","client_lead": "client_lead","customer_support": "customer_support","hiring_support": "hiring_support","meeting_booking": "meeting_booking","general_chat": "general_chat","unrelated_query": "unrelated_query",},)

graph.add_edge("client_lead", "response_generator")graph.add_edge("customer_support", "response_generator")graph.add_edge("hiring_support", "response_generator")graph.add_edge("meeting_booking", "response_generator")graph.add_edge("general_chat", "response_generator")graph.add_edge("unrelated_query", END)graph.add_edge("response_generator", END)

chatbot = graph.compile(checkpointer=checkpointer)

def get_fixed_response_options(active_collection: Optional[str],pending_field: Optional[str],profile: Optional[dict] = None,thread_id: Optional[str] = None,) -> list[dict]:if active_collection != "meeting_booking":return []
...
@traceabledef send_message(user_input: str, thread_id: str = "test_user"):thread_id_var.set(thread_id)node_name_var.set("general")config = {"configurable": {"thread_id": thread_id}}
